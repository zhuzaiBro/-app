...
function ScrolItem({ title = "", route, index = 0, length = 5, navigation,
    ...
    return (
        <Pressable onLayout={(e) => {
            ...
            <Text numberOfLines={1} style={{ marginLeft: ROATE(5), fontSize: ROATE(14), width: ROATE(148), color: "rgba(255,255,255,0.9)", }}>#国风顶流新中式{title}</Text>
...
export default function ScrollList({route, navigation, getScrollListData, duration = 20000, ListData = [] }: {
    ...
    // for (let i = 0; i < 5; i++) {
    //     list.push(<ScrolItem route={route} navigation={navigation} title={i + ""} key={i} />)
    // }
    useEffect(() => {
        (async() => {
